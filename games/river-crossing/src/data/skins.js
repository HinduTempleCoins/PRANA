...
//
// A "skin" is a hopper palette + body-shape variant. The shape DELIBERATELY MIRRORS an
...
//   { itemId, name, shape, palette: { body, accent, glow } }
//     - itemId : ItemRegistry cosmetic id, MUST be >= 30_000 (the cosmetics range).
//     - shape  : hopper-shape variant key ('round' | 'diamond'), purely visual.
//     - palette: luminous `body` -> `accent` ring -> `glow` aura tint (the dark-field /
//                bright-core motif — a glowing spark crossing a dark river).
//
...
export const HOPPER_SHAPES = ['round', 'diamond'];

const PALETTE_KEYS = ['body', 'accent', 'glow'];
...
export function normalizeSkin(raw, index = 0) {
  ...
  const shape = HOPPER_SHAPES.includes(raw.shape) ? raw.shape : 'round';
  return {
    itemId,
    name: String(raw.name ?? `Skin #${itemId}`),
    shape,
    palette: { body: palette.body, accent: palette.accent, glow: palette.glow },
  };
}
