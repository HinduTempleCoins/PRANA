...
//
// A "skin" is a well/stela palette theme. The shape DELIBERATELY MIRRORS an on-chain
...
//   { itemId, name, palette: { well, grid, glow } }
//     - itemId : ItemRegistry cosmetic id, MUST be >= 30_000 (the cosmetics range).
//     - palette: `well` dark backdrop, `grid` faint gridline, `glow` accent for cleared rows.
//                Individual stela colours come from logic/stack.js PIECES (gameplay, not skin).
//
...
const PALETTE_KEYS = ['well', 'grid', 'glow'];
...
export function normalizeSkin(raw, index = 0) {
  ...
  return {
    ...
    palette: { well: palette.well, grid: palette.grid, glow: palette.glow },
  };
}
