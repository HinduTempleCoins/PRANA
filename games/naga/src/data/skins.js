...
//
// A "skin" is a snake palette + head-shape variant. The shape DELIBERATELY MIRRORS an
...
//   { itemId, name, head, palette: { head, body, glow } }
//     - itemId : ItemRegistry cosmetic id, MUST be >= 30_000 (the cosmetics range).
//     - head   : head-shape variant key ('round' | 'diamond'), purely visual.
//     - palette: dark `body` base -> luminous `head` -> `glow` orb tint (the dark-field /
//                bright-core motif).
//
...
export const HEAD_SHAPES = ['round', 'diamond'];

const PALETTE_KEYS = ['head', 'body', 'glow'];
...
export function normalizeSkin(raw, index = 0) {
  ...
  const head = HEAD_SHAPES.includes(raw.head) ? raw.head : 'round';
  return {
    itemId,
    name: String(raw.name ?? `Skin #${itemId}`),
    head,
    palette: { head: palette.head, body: palette.body, glow: palette.glow },
  };
}
