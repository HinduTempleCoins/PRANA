...
//
// A "skin" is a hopper palette + trail variant. The shape DELIBERATELY MIRRORS an on-chain
// cosmetic item from contracts/contracts/ItemRegistry.sol, whose id-range convention
// reserves 30_000 .. max for cosmetics (COSMETIC_MINTER_ROLE). So a skin maps 1:1 onto an
// ItemRegistry cosmetic id and can later be ownership-gated by the wallet hook.
//
// Canonical skin shape (the ONLY thing the game reads):
//   { itemId, name, trail, palette: { body, edge, glow } }
//     - itemId : ItemRegistry cosmetic id, MUST be >= 30_000 (the cosmetics range).
//     - trail  : bounce-trail style key ('spark' | 'ribbon'), purely visual.
//     - palette: dark `body` base -> bright `edge` bevel -> `glow` trail tint.
//
...
export const TRAIL_STYLES = ['spark', 'ribbon'];

const PALETTE_KEYS = ['body', 'edge', 'glow'];
...
export function normalizeSkin(raw, index = 0) {
  ...
  const trail = TRAIL_STYLES.includes(raw.trail) ? raw.trail : 'spark';
  return {
    itemId,
    name: String(raw.name ?? `Skin #${itemId}`),
    trail,
    palette: { body: palette.body, edge: palette.edge, glow: palette.glow },
  };
}
