...
//
// A "skin" is a paddle/ball/backdrop palette theme. The shape DELIBERATELY MIRRORS an
// on-chain cosmetic item from contracts/contracts/ItemRegistry.sol, whose id-range convention
...
//   { itemId, name, palette: { bg, paddle, ball } }
//     - itemId : ItemRegistry cosmetic id, MUST be >= 30_000 (the cosmetics range).
//     - palette: `bg` backdrop, `paddle` paddle colour, `ball` ball colour.
//                Brick colours come from the HP tiers in logic/bounce.js (gameplay, not skin).
//
...
const PALETTE_KEYS = ['bg', 'paddle', 'ball'];
...
export function normalizeSkin(raw, index = 0) {
  ...
  return {
    ...
    palette: { bg: palette.bg, paddle: palette.paddle, ball: palette.ball },
  };
}
