...
/// @title UniswapV2Pair
/// @notice Canonical Uniswap V2 core pair ported from Solidity 0.5.16 to 0.8.24.
/// @dev PORTING NOTES:
///      - SafeMath removed; reserve/k math uses native checked arithmetic so insufficient
///        amounts revert instead of silently wrapping (the x*y=k invariant check in `swap`
///        and the balance subtractions are all checked).
///      - The 0.5.16 source intentionally relied on uint112 / uint32 *wrapping* for the
///        price accumulators and the `timeElapsed` computation. Those specific spots are
///        wrapped in `unchecked { }` to preserve identical behavior under 0.8.x.
...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    function _update(uint256 balance0, uint256 balance1, uint112 _reserve0, uint112 _reserve1) private {
        ...
        uint32 timeElapsed;
        // The accumulators are *designed* to overflow/wrap (canonical V2 behavior). Keep that.
        unchecked {
